...
- **Export** (`exportStoredIdentityBackup(userId, passphrase)` / `exportIdentityBackup`, `client/src/lib/e2ee/crypto.ts`): the caller supplies a dedicated backup passphrase (separate from OIDC sign-in — there is no app password to reuse). There is **no minimum length or complexity requirement**; the UI shows a non-blocking strength indicator only. A 256-bit key is derived with **scrypt** (`@noble/hashes/scrypt`, `N=2^17, r=8, p=1`, random 16-byte salt), then the identity JSON is encrypted with **AES-256-GCM** (random 12-byte nonce). The envelope has no format-version field: `{ "alg": "scrypt-aes256gcm", "kdf": { "n", "r", "p", "salt" }, "nonce", "ciphertext" }`. The file downloads directly (`Blob` + `<a download>`), never through the clipboard.
- **Import** (`importStoredIdentityBackup(userId, raw, passphrase)` / `importIdentityBackup`): re-derives the key from the embedded salt/params and decrypts. A file missing the `alg`/`kdf`/`nonce`/`ciphertext` shape (including the legacy unencrypted export format this replaces) is rejected up front with "unsupported or legacy backup format"; a wrong passphrase or corrupted ciphertext both fail at the AES-GCM auth tag and surface the same generic "wrong passphrase or corrupted backup file" error, so the message itself is never a passphrase oracle.
