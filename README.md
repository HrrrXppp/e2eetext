...
- That allowlist only applies once the workspace is marked trusted, which normally requires clicking through an interactive one-time dialog recorded in `~/.claude.json`. Since the container never runs `claude` interactively, `entrypoint.sh` pre-seeds `hasTrustDialogAccepted: true` for `/workspace/e2eetext` in `~/.claude.json` before starting the loop, so `permissions.allow` isn't silently ignored.
