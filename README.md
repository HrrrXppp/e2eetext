...
- **Import** (`importStoredIdentityBackup(userId, raw, passphrase)` / `importIdentityBackup`): re-derives the key from the embedded salt/params and decrypts. A file missing the `alg`/`kdf`/`nonce`/`ciphertext` shape (including the legacy unencrypted export format this replaces) is rejected up front with "unsupported or legacy backup format"; a wrong passphrase or corrupted ciphertext both fail at the AES-GCM auth tag and surface the same generic "wrong passphrase or corrupted file" error, so the message itself is never a passphrase oracle.
- **Multi-device / re-import collision handling**: before overwriting the identity stored on this device, import compares the backup's public key against the account's current server-side key (`fetchIdentityPublicKey`). If they differ, the UI requires an explicit "Overwrite and restore" confirmation and explains that other participants' existing chat-key wraps are bound to the specific public key and may need to be re-shared — restoring from a backup recovers your own identity after device loss, it is not a way to keep multiple devices in sync.
