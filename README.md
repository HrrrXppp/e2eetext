...
The Edit name dialog (opened from the Name button in the header) exposes **Back up key** and **Restore key** buttons whenever a user is signed in, and a one-time prompt offers the same actions immediately after a brand-new local identity keypair is generated (new account, or first sign-in on a new device) — the prompt is not gated on a persisted "seen" flag, so it fires once per generation event and never reappears on later sign-ins.
