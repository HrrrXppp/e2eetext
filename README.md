...
Private identity ciphertext is stored per account in `localStorage` (`e2ee_identity_v1:{userId}`) and encrypted with a **non-extractable AES-256-GCM** wrapping key kept in IndexedDB (Web Crypto). That blocks XSS from reading raw wrapping-key bytes out of storage. Active same-origin XSS can still invoke decrypt APIs while the tab is unlocked — a passphrase lock or isolated crypto origin would be needed to close that remaining gap. Sign-out clears that account's local E2EE material.

### Private key export / import

The User Settings dialog (opened from the Name button in the header) exposes **Back up key** and **Restore key** buttons whenever a user is signed in, and a one-time prompt offers the same actions immediately after a brand-new local identity keypair is generated (new account, or first sign-in on a new device) — the prompt is not gated on a persisted "seen" flag, so it fires once per generation event and never reappears on later sign-ins.

- **Export** (`exportStoredIdentityBackup(userId, passphrase)` / `exportIdentityBackup(identity, passphrase, userId)`, `client/src/lib/e2ee/crypto.ts`): the caller supplies a dedicated backup passphrase (separate from OIDC sign-in — there is no app password to reuse). There is **no minimum length or complexity requirement**; the UI shows a non-blocking strength indicator only. A 256-bit key is derived with **scrypt** (`@noble/hashes/scrypt`, `N=2^17, r=8, p=1`, random 16-byte salt), then the identity JSON — plus the owning account's `userId`, so each backup is bound to the account it came from — is encrypted with **AES-256-GCM** (random 12-byte nonce). The envelope has no format-version field: `{ "alg": "scrypt-aes256gcm", "kdf": { "n", "r", "p", "salt" }, "nonce", "ciphertext" }`. The file downloads directly (`Blob` + `<a download>`), never through the clipboard.
- **Import** (`importStoredIdentityBackup(userId, raw, passphrase)` / `importIdentityBackup(raw, passphrase, expectedUserId)`): re-derives the key from the embedded salt/params and decrypts. A file missing the `alg`/`kdf`/`nonce`/`ciphertext` shape (including the legacy unencrypted export format this replaces) is rejected up front with "unsupported or legacy backup format"; a wrong passphrase or corrupted ciphertext both fail at the AES-GCM auth tag and surface the same generic "wrong passphrase or corrupted backup file" error, so the message itself is never a passphrase oracle. Once decrypted, the embedded `userId` is checked against the currently signed-in account; a backup exported from a different account is rejected with a distinct "this backup belongs to a different account" error rather than silently importing (or being lumped in with the generic wrong-passphrase error) — the `userId` binding lives inside the authenticated ciphertext, so it can't be edited around in the plaintext envelope.
- **Multi-device / re-import collision handling**: before overwriting the identity stored on this device, import compares the backup's public key against both the identity currently loaded on this device (`loadStoredIdentity`) and the account's current server-side key (`fetchIdentityPublicKey`, treating a 404/unreachable server as "no conflict"). If either differs, the UI requires an explicit "Overwrite and restore" confirmation and explains that other participants' existing chat-key wraps are bound to the specific public key and may need to be re-shared — restoring from a backup recovers your own identity after device loss, it is not a way to keep multiple devices in sync.
...
├── entrypoint.sh        # clone/fetch dev, run `claude -p` on a timer, exec passthrough for ad hoc/interactive commands
...
`docker run --rm <image> <cmd>` (or any command passed to the container) is exec'd directly instead of starting the loop — e.g. `docker run --rm claude-runner claude --version` — which is how the image itself is smoke-tested. With no command, the entrypoint clones/fetches the repo into a persistent `/workspace` volume and runs `claude -p "Run the GitHub issue/PR ticket-processing cycle described in CLAUDE.md."` every `CYCLE_INTERVAL_SECONDS` (default 300), logging and retrying on the next interval if a cycle fails rather than crash-looping the container. Before each cycle (and once at startup), the checkout is reset hard to `origin/dev` and any untracked files are cleaned — `/workspace` is a disposable mirror, not a place for durable state, so leftover local changes (e.g. from a killed cycle) are discarded rather than left to break the next `git checkout` with a conflict.

`interactive` is the one non-passthrough command: `docker compose -f maintenance/claude-runner/docker-compose.yml run --rm claude-runner interactive` (or `docker run -it --rm --env-file .env -v claude-runner-workspace:/workspace -v claude-runner-state:/home/claude/.claude claude-runner interactive`) runs the same setup the loop uses — env var checks, git config, clone/fetch, workspace-trust seeding — and then drops into an interactive `claude` session (instead of the unattended `claude -p` loop) in that same `/workspace/e2eetext` checkout, so the ticket-processing prompt (or anything else) can be driven by hand. Needs a tty/stdin attached, which both `docker run -it` and `docker compose run` allocate by default.
...
- That allowlist only applies once the workspace is marked trusted, which normally requires clicking through an interactive one-time dialog recorded in `~/.claude.json`. The unattended loop never runs `claude` interactively, so `entrypoint.sh` pre-seeds `hasTrustDialogAccepted: true` for `/workspace/e2eetext` in `~/.claude.json` before starting the loop (and before handing off to `interactive` mode, so `permissions.allow` isn't silently ignored there either, and a human running `interactive` isn't stopped by the trust dialog).
