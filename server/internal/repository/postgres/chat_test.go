...
func TestChatRepository_ListMembers(t *testing.T) {
	...
	joinedAt := time.Date(2026, 6, 11, 12, 0, 0, 0, time.UTC)

	rows := sqlmock.NewRows([]string{"user_id", "is_admin", "created_at"}).
		AddRow(adminID, true, joinedAt).
		AddRow(memberID, false, joinedAt)

	mock.ExpectQuery(`SELECT uc\.user_id, \(ca\.user_id IS NOT NULL\) AS is_admin, uc\.created_at[\s\S]+ORDER BY uc\.created_at ASC`).
		WithArgs(chatID).
		WillReturnRows(rows)

	members, err := repo.ListMembers(context.Background(), chatID)
	if err != nil {
	...
	if members[0].UserID != adminID || !members[0].IsAdmin {
		t.Errorf("members[0] = %+v", members[0])
	}
	if members[1].UserID != memberID || members[1].IsAdmin {
