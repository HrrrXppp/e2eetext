...
func (h *UserHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// Some providers (Apple) never put a name in the ID token at all — the
	// name is only ever available out-of-band, once, at OAuth callback time
	// (see AuthService.CompleteLogin). The client relays it here. Only fill
	// the gap when the verified ID token itself carried no name, so a
	// verified claim always wins over this unverifiable, client-supplied
	// value.
	if name := strings.TrimSpace(req.Name); name != "" && tokenUser.Name == "" {
		tokenUser.Name = name
	}

	user, err := h.service.CreateFromToken(r.Context(), tokenUser, req.SkipProfile)
	if err != nil {
